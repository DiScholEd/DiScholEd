...
(:~~
 ...
 : (default: 8000, but check where your server is running).
  declare variable $config:webcomponents := "dev"; declare variable $config:webcomponents := "2.4.2";:)
...
(:
 ...
 :)
declare variable $config:default-template :="letters.html";
...
declare variable $config:facets := [
    ...
    "output": function($label) {
            switch($label)
                case "annotation_in_progress" return 'Annotation in progress'
                case "raw_transcription" return 'Raw transcription'
                case "final_publication" return 'Final publication'
                default return $label
        }
    },
    map {
...
(:~
 ...
 : arguments.
 :)
declare variable $config:tex-command := function($file) { ("pdflatex", "-interaction=nonstopmode", $file)};
...
(:~
 ...
 :)
declare variable $config:default-odd :="myodd.odd";
...
(:~
 ...
 :)
declare variable $config:odd-available :=("myodd.odd");
