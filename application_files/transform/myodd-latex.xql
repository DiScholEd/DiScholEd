...
module namespace model="http://www.tei-c.org/pm/models/myodd/latex";
...
declare %private function model:template-TEI($config as map(*), $node as node()*, $params as map(*)) {
    ``[\documentclass[12pt,a4paper,fromalign=right]{book}
...
(: generated template function for element spec: del :)
declare %private function model:template-del2($config as map(*), $node as node()*, $params as map(*)) {
    ``[\sout{`{string-join($config?apply-children($config, $node, $params?content))}`}]``
};
...
declare %private function model:template-closer2($config as map(*), $node as node()*, $params as map(*)) {
    ``[`{string-join($config?apply-children($config, $node, $params?content))}`
]``
};
(: generated template function for element spec: closer :)
...
declare %private function model:template-lb3($config as map(*), $node as node()*, $params as map(*)) {
    ``[\\]``
};
...
declare %private function model:template-add($config as map(*), $node as node()*, $params as map(*)) {
    ``[\textsf{\hl{`{string-join($config?apply-children($config, $node, $params?content))}`}}]``
};
(: generated template function for element spec: add :)
            ...
            return
                            ...
                            if ($parameters?mode='index') then
                                (
                                    ...
                                    if (descendant::link) then
                                        ...
                                        (),
                                    if (child::note) then
                                        latex:block($config, ., ("tei-item5", css:map-rend-to-class(.)), note)
                                    else
                                        (),
                                    if (child::desc) then
                                        latex:block($config, ., ("tei-item6", css:map-rend-to-class(.)), desc)
                                    else
                                        (),
                                    if (child::ref) then
                                        let $params := 
                                        ...
                                        return
                                                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-item7", css:map-rend-to-class(.)), $content)
                                    else
                                        ()
                                )
                                ...
                                if ($parameters?mode='list') then
                                    ...
                                    if (preceding-sibling::label) then
                                        latex:inline($config, ., ("tei-item9", css:map-rend-to-class(.)), .)
                                    else
                                        ...
                                        return
                                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-item10", css:map-rend-to-class(.)), $content)
                    case element(figure) return
                        latex:figure($config, ., ("tei-figure1", css:map-rend-to-class(.)), ., ())
                    case element(teiHeader) return
                        (
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader2", css:map-rend-to-class(.)), $content),
                            if (.//correspDesc) then
                                let $params := 
                                    map {
                                        "author": .//correspAction[@type='sent']/persName,
                                        "recipient": .//correspAction[@type='received']/persName,
                                        "content": .
                                    }
                                ...
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader3", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (not(.//correspDesc)) then
                                ...
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader4", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            let $params := 
                                map {
                                    "origin": .//history/origin/p[@xml:lang='en'],
                                    "content": .
                                }
                            ...
                            let $params := 
                                map {
                                    ...
                                    "number": string-join(.//supportDesc/foliation/p[@xml:lang='en'], '. '),
                                    "conservation": .//supportDesc/condition/p[@xml:lang='en'],
                                    "annex": if (@accMat) then concat("\item Appendix: ", .//accMat/p[@xml:lang='en']) else '',
                                    "content": .
                                }

                                                        let $content := 
                                model:template-teiHeader6($config, ., $params)
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader6", css:map-rend-to-class(.)), $content),
                            let $params := 
                                map {
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader7", css:map-rend-to-class(.)), $content),
                            let $params := 
                                map {
                                    "projet": .//projectDesc/p[@xml:lang='en'],
                                    "responsable": .//principal/persName,
                                    "editor": .//respStmt[1]/persName,
                                    "correction": .//correction/p[@xml:lang='en'],
                                    "hyphenation": .//hyphenation/p[@xml:lang='en'],
                                    "licence": .//publicationStmt/availability/licence,
                                    "content": .
                                }
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader8", css:map-rend-to-class(.)), $content),
                            let $params := 
                                map {
                                    "content": .
                                }
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader9", css:map-rend-to-class(.)), $content)
                        )
                    ...
                    case element(supplied) return
                        if ($parameters?mode="corrections") then
                            (
                                if (@reason) then
                                    latex:alternate($config, ., ("tei-supplied1", css:map-rend-to-class(.)), ., ., concat("The characters has been supplied because the text is ", @reason))
                                else
                                    (),
                                if (not(@reason)) then
                                    latex:alternate($config, ., ("tei-supplied2", css:map-rend-to-class(.)), ., ., "The characters has been supplied")
                                else
                                    ()
                            )
                    ...
                    case element(milestone) return
                        if (@type="line") then
                            ...
                            return
                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-milestone1", css:map-rend-to-class(.)), $content)
                        else
                            latex:inline($config, ., ("tei-milestone2", css:map-rend-to-class(.)), .)
                    ...
                    case element(label) return
                        if ($parameters?mode='index' and parent::event) then
                            latex:inline($config, ., ("tei-label1", css:map-rend-to-class(.)), .)
                        else
                            if ($parameters?mode='index') then
                                latex:block($config, ., ("tei-label2", css:map-rend-to-class(.)), .)
                            else
                                if ($parameters?mode='list') then
                                    latex:block($config, ., ("tei-label3", css:map-rend-to-class(.)), .)
                                else
                                    if (@hand and $parameters?mode="corrections") then
                                        latex:alternate($config, ., ("tei-label4", css:map-rend-to-class(.)), ., ., id(substring-after(@hand, '#'), root($parameters?root))/p[@xml:lang="en"])
                                    else
                                        if (parent::item) then
                                            let $params := 
                                                map {
                                            ...
                                            return
                                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-label5", css:map-rend-to-class(.)), $content)
                                        else
                                            if (parent::list) then
                                                let $params := 
                                                    map {
                                                ...
                                                return
                                                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-label6", css:map-rend-to-class(.)), $content)
                                            else
                                                $config?apply($config, ./node())
                                ...
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-signed($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-signed1", css:map-rend-to-class(.)), $content)
                            else
                                (),
                                ...
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-signed2($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-signed2", css:map-rend-to-class(.)), $content)
                            else
                                (),
                                ...
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-signed3($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-signed3", css:map-rend-to-class(.)), $content)
                            else
                                (),
                                ...
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-signed4($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-signed4", css:map-rend-to-class(.)), $content)
                            else
                                (),
                                ...
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-signed5($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-signed5", css:map-rend-to-class(.)), $content)
                            else
                                ()
                        )
                    ...
                    case element(anchor) return
                        if (preceding-sibling::delSpan) then
                            latex:block($config, ., ("tei-anchor1", css:map-rend-to-class(.)), "[End of the deletion]")
                        else
                    ...
                    case element(choice) return
                        if ($parameters?mode='corrections') then
                            latex:inline($config, ., ("tei-choice1", css:map-rend-to-class(.)), abbr[1])
                        else
                            if ($parameters?mode="original" or $parameters?mode="entities") then
                                latex:inline($config, ., ("tei-choice2", css:map-rend-to-class(.)), expan[1])
                            else
                                if (unclear) then
                                    latex:inline($config, ., ("tei-choice3", css:map-rend-to-class(.)), unclear[1])
                                else
                                    if (sic and corr) then
                                        latex:alternate($config, ., ("tei-choice6", css:map-rend-to-class(.)), ., corr[1], sic[1])
                                    else
                    ...
                    case element(hi) return
                        (
                            ...
                            if (@rend="latin") then
                                ...
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-hi2", css:map-rend-to-class(.)), $content)
                            else
                                (),
                                ...
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-hi3($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-hi3", css:map-rend-to-class(.)), $content)
                            else
                                ()
                        )
                    ...
                    case element(note) return
                        (
                            if (@place="top-margin-left") then
                                ...
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-note1", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@place="bottom-right") then
                                let $params := 
                                    map {
                                        "content": .
                                    }
                            ...
                            if (@place="bottom-left") then
                                ...
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-note3", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@resp) then
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-note4($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-note4", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@place="top-margin-right") then
                                let $params := 
                                    map {
                                        "content": .
                                    }
                                                                ...
                                                                let $content := 
                                    model:template-note5($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-note5", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@place="top") then
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-note6($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-note6", css:map-rend-to-class(.)), $content)
                            else
                                (),
                                ...
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-note7($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-note7", css:map-rend-to-class(.)), $content)
                            else
                                ()
                        )
                                ...
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-dateline($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-dateline1", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@rend="align(center)") then
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-dateline2($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-dateline2", css:map-rend-to-class(.)), $content)
                            else
                                (),
                                ...
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-dateline3($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-dateline3", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@rend="indent") then
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-dateline4($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-dateline4", css:map-rend-to-class(.)), $content)
                            else
                                ()
                        )
                            ...
                            let $params := 
                                map {
                                    "content": .
                                                        ...
                                                        let $content := 
                                model:template-del2($config, ., $params)
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-del2", css:map-rend-to-class(.)), $content)
                        else
                            if ($parameters?mode='entities' or $parameters?mode='original') then
                                latex:omit($config, ., ("tei-del3", css:map-rend-to-class(.)), .)
                            else
                                if (@rend='overwritten' and $parameters?mode='corrections') then
                                    ...
                                    if (@rend='strikethrough' and $parameters?mode='corrections') then
                                        latex:inline($config, ., ("tei-del6", css:map-rend-to-class(.)), .)
                                    else
                                        $config?apply($config, ./node())
                    case element(trailer) return
                        ...
                        return
                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-trailer", css:map-rend-to-class(.)), $content)
                    ...
                    case element(revisionDesc) return
                        if ($parameters?header='short' and @status) then
                            ...
                            return
                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-revisionDesc1", "header-short", css:map-rend-to-class(.)), $content)
                        else
                            latex:omit($config, ., ("tei-revisionDesc2", css:map-rend-to-class(.)), .)
                    ...
                    case element(subst) return
                        if ((del[@rend="overwritten"] and add) and $parameters?mode="corrections") then
                            latex:alternate($config, ., ("tei-subst3", css:map-rend-to-class(.)), ., add, concat("This text has been overwritten: ", del))
                        else
                            if (del[@rend="overwritten"] and add) then
                                latex:alternate($config, ., ("tei-subst4", css:map-rend-to-class(.)), ., add, del)
                            else
                                $config?apply($config, ./node())
                    ...
                    case element(speaker) return
                        if (@rend="underline") then
                            latex:inline($config, ., ("tei-speaker1", css:map-rend-to-class(.)), .)
                        else
                            latex:block($config, ., ("tei-speaker2", css:map-rend-to-class(.)), .)
                    ...
                    case element(rs) return
                        if ($parameters?mode="facets" and @type="bi_place") then
                            latex:omit($config, ., ("tei-rs1", css:map-rend-to-class(.)), .)
                        else
                    ...
                    case element(profileDesc) return
                        if ($parameters?mode="corresp" and child::correspContext) then
                            ...
                            if ($parameters?header='short' and child::abstract) then
                                latex:block($config, ., ("tei-profileDesc2", css:map-rend-to-class(.)), abstract/p)
                            else
                                if ($parameters?header='short' and child::correspDesc/correspAction[@type="sent"]/placeName) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc3", "header-short", css:map-rend-to-class(.)), $content)
                                else
                                    latex:omit($config, ., ("tei-profileDesc4", css:map-rend-to-class(.)), .)
                    case element(email) return
                        latex:inline($config, ., ("tei-email", css:map-rend-to-class(.)), .)
                    case element(text) return
                        if ($parameters?mode='list') then
                            latex:block($config, ., ("tei-text1", css:map-rend-to-class(.)), /body)
                        else
                        ...
                        if (@ana) then
                            (
                                if ($parameters?mode='corrections') then
                                    ...
                                    (),
                                if ($parameters?mode="original" or $parameters?mode='entities') then
                                    latex:inline($config, ., ("tei-abbr2", css:map-rend-to-class(.)), substring-after(@ana, "#"))
                                else
                                    ()
                            )
                    ...
                    case element(table) return
                        if (@rend="align(center)") then
                            latex:table($config, ., ("tei-table1", css:map-rend-to-class(.)), ., map {})
                        else
                            latex:table($config, ., ("tei-table2", css:map-rend-to-class(.)), ., map {})
                    ...
                    case element(licence) return
                        if (@target) then
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-licence1", css:map-rend-to-class(.)), $content)
                        else
                            latex:block($config, ., ("tei-licence2", css:map-rend-to-class(.)), .)
                    ...
                    case element(address) return
                        if (@rend='align(right)') then
                            latex:block($config, ., ("tei-address1", css:map-rend-to-class(.)), .)
                        else
                            if (@rend='align(left)') then
                                latex:block($config, ., ("tei-address2", css:map-rend-to-class(.)), .)
                            else
                                if (@rend='align(center)') then
                                    latex:block($config, ., ("tei-address3", css:map-rend-to-class(.)), .)
                                else
                                    (: More than one model without predicate found for ident address. Choosing first one. :)
                                    latex:block($config, ., ("tei-address4", css:map-rend-to-class(.)), .)
                        ...
                        let $params := 
                            map {
                                "content": .
                                                ...
                                                let $content := 
                            model:template-closer2($config, ., $params)
                        return
                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-closer2", css:map-rend-to-class(.)), $content)
                    ...
                    case element(list) return
                        ...
                        return
                                                latex:list(map:merge(($config, map:entry("template", true()))), ., ("tei-list", css:map-rend-to-class(.)), $content, ())
                                ...
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-p($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-p1", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@rend="align(center)") then
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-p2($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-p2", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@rend="align(left)") then
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-p3($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-p3", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@rend="indent") then
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-p4($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-p4", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (not(@rend) and not(ancestor::teiHeader)) then
                                let $params := 
                                    map {
                                        "content": .
                                    }
                                                                ...
                                                                let $content := 
                                    model:template-p5($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-p5", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (ancestor::teiHeader) then
                                latex:inline($config, ., ("tei-p6", css:map-rend-to-class(.)), .)
                            else
                                ()
                        )
                    ...
                    case element(lb) return
                        if (preceding-sibling::item) then
                            latex:break($config, ., ("tei-lb1", css:map-rend-to-class(.)), ., 'line', @n)
                        else
                            if (not(@break="no") and ($parameters?mode="original" or $parameters?mode="entities")) then
                                latex:inline($config, ., ("tei-lb2", css:map-rend-to-class(.)), " ")
                            else
                                if ($parameters?mode='corrections') then
                                    latex:break($config, ., css:get-rendition(., ("tei-lb4", css:map-rend-to-class(.))), ., 'line', @n)
                                else
                                    (: latex rendition :)
                                    let $params := 
                                        map {
                                            "content": .
                                                                        ...
                                                                        let $content := 
                                        model:template-lb3($config, ., $params)
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-lb3", css:map-rend-to-class(.)), $content)
                    ...
                    case element(name) return
                        if ($parameters?mode='index') then
                            latex:block($config, ., ("tei-name1", css:map-rend-to-class(.)), .)
                        else
                            latex:inline($config, ., ("tei-name2", css:map-rend-to-class(.)), .)
                    case element(front) return
                        latex:block($config, ., ("tei-front", css:map-rend-to-class(.)), .)
                    case element(lg) return
                        latex:block($config, ., ("tei-lg", css:map-rend-to-class(.)), .)
                    case element(publicationStmt) return
                        latex:inline($config, ., ("tei-publicationStmt", css:map-rend-to-class(.)), .)
                    ...
                    case element(postscript) return
                        ...
                        return
                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-postscript", css:map-rend-to-class(.)), $content)
                        ...
                        if ($parameters?mode='facets') then
                            (
                                if (descendant::persName) then
                                    latex:heading($config, ., ("tei-div1", css:map-rend-to-class(.)), 'Persons', 2)
                                else
                                    (),
                                latex:block($config, ., ("tei-div2", css:map-rend-to-class(.)), for $n in .//persName group by $ref := $n/@ref order by $ref return $n[1]),
                                if (descendant::orgName) then
                                    latex:heading($config, ., ("tei-div3", css:map-rend-to-class(.)), 'Organisations/Groups', 2)
                                else
                                    (),
                                latex:block($config, ., ("tei-div4", css:map-rend-to-class(.)), for $n in .//orgName group by $ref := $n/@ref order by $ref return $n[1]),
                                if (descendant::title[@ref]) then
                                    latex:heading($config, ., ("tei-div5", css:map-rend-to-class(.)), "Bibliography", 2)
                                else
                                    (),
                                latex:block($config, ., ("tei-div6", css:map-rend-to-class(.)), for $n in .//title[@ref] group by $ref := $n/@ref order by $ref return $n[1]),
                                if (descendant::rs) then
                                    latex:heading($config, ., ("tei-div7", css:map-rend-to-class(.)), "Undefined references", 2)
                                else
                                    (),
                                latex:block($config, ., ("tei-div8", css:map-rend-to-class(.)), for $n in .//rs group by $ref := $n/@ref order by $ref return $n[1]),
                                if (descendant::term) then
                                    latex:heading($config, ., ("tei-div9", css:map-rend-to-class(.)), "Terms", 2)
                                else
                                    (),
                                latex:block($config, ., ("tei-div10", css:map-rend-to-class(.)), for $n in .//term group by $ref := $n/@ref order by $ref return $n[1])
                            )

                        else
                            if ($parameters?mode='facets' and parent::body) then
                                (: facets rendition :)
                                latex:block($config, ., ("tei-div15", css:map-rend-to-class(.)), .)
                            else
                                if (@type='title_page') then
                                    latex:block($config, ., ("tei-div16", css:map-rend-to-class(.)), .)
                                else
                                    if (parent::body) then
                                        latex:section($config, ., ("tei-div17", css:map-rend-to-class(.)), .)
                                    else
                                        latex:block($config, ., ("tei-div18", css:map-rend-to-class(.)), .)
                    ...
                    case element(ref) return
                        if (parent::bibl) then
                            ...
                            return
                                                        latex:listItem(map:merge(($config, map:entry("template", true()))), ., ("tei-ref1", css:map-rend-to-class(.)), $content, ())
                        else
                            if (id(substring-after(@target, '#'), root($parameters?root))/div[@type="chapter"]) then
                                latex:omit($config, ., ("tei-ref2", css:map-rend-to-class(.)), .)
                            else
                                if (not(@target)) then
                            ...
                            let $params := 
                                map {
                                    "content": .
                                                        ...
                                                        let $content := 
                                model:template-add($config, ., $params)
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-add1", css:map-rend-to-class(.)), $content)
                        else
                            if (@hand and $parameters?mode='corrections') then
                                (: for diplomatic view :)
                                let $params := 
                                ...
                                return
                                                                latex:alternate(map:merge(($config, map:entry("template", true()))), ., ("tei-add3", css:map-rend-to-class(.)), $content, ., concat("Additions made by: ", id(substring-after(@hand, '#'), root($parameters?root))/p[@xml:lang="en"]))
                            else
                                if ($parameters?mode='corrections') then
                                    (: for diplomatic view :)
                                    latex:alternate($config, ., ("tei-add4", css:map-rend-to-class(.)), ., ., "Additions made by the major hand")
                                else
                                    (: default model :)
                                    latex:inline($config, ., ("tei-add5", css:map-rend-to-class(.)), .)
                    case element(docDate) return
                        latex:inline($config, ., ("tei-docDate", css:map-rend-to-class(.)), .)
                    case element(head) return
                        (
                            if (contains(@rend, 'center') and contains(@rend, 'underline')) then
                                let $params := 
                                    map {
                                        "content": .
                                    }
                                                                ...
                                                                let $content := 
                                    model:template-head($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-head1", css:map-rend-to-class(.)), $content)
                            else
                                ()
                        )
                    ...
                    case element(bibl) return
                        if (parent::person or parent::place) then
                            latex:inline($config, ., ("tei-bibl1", css:map-rend-to-class(.)), ref)
                        else
                                ...
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-salute($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-salute1", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@rend="align(center)") then
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-salute2($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-salute2", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@rend="align(left)") then
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-salute3($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-salute3", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@rend="indent") then
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-salute4($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-salute4", css:map-rend-to-class(.)), $content)
                            else
                                ()
                        )
                    ...
                    case element(unclear) return
                        if ($parameters?mode="corrections") then
                            (
                                if (@reason) then
                                    latex:alternate($config, ., ("tei-unclear1", css:map-rend-to-class(.)), ., ., concat("The characters are unclear because the text is ", @reason))
                                else
                                    (),
                                if (not(@reason)) then
                                    latex:alternate($config, ., ("tei-unclear2", css:map-rend-to-class(.)), ., ., "Unclear character(s)")
                                else
                                    ()
                            )
                    ...
                    case element(argument) return
                        (: default rendition :)
                        latex:block($config, ., ("tei-argument2", css:map-rend-to-class(.)), .)
                    case element(date) return
                        if (text()) then
                            ...
                            if (@when) then
                                ...
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-date2", css:map-rend-to-class(.)), $content)
                            else
                                if (@when-iso) then
                                    latex:inline($config, ., ("tei-date3", css:map-rend-to-class(.)), format-date(@when-iso, '[D10] [MNn] [Y]', 'en', 'AD', 'GB'))
                                else
                                    if (@when-iso) then
                                        let $params := 
                                            map {
                                        ...
                                        return
                                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-date4", css:map-rend-to-class(.)), $content)
                                    else
                                        if (parent::affiliation) then
                                            ...
                                            return
                                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-date5", css:map-rend-to-class(.)), $content)
                                        else
                                            $config?apply($config, ./node())
                        ...
                        if ($parameters?mode='facets') then
                            ...
                            if ($parameters?mode='entities' and (@type="bi" and id(substring-after(@ref, '#'), doc("/db/apps/discholed/data/bi/indexes/Werkindex.xml"))) or (@type="pec" and id(substring-after(@ref, '#'), doc("/db/apps/discholed/data/pec/index/Index_biblio.xml"))) or (@type="ehri" and id(substring-after(@ref, '#'), root($parameters?root)))) then
                                ...
                                if (@rend="align(right) underline") then
                                    latex:block($config, ., ("tei-title18", css:map-rend-to-class(.)), .)
                                else
                                    if (@rend="align(right)") then
                                        latex:block($config, ., ("tei-title19", css:map-rend-to-class(.)), .)
                                    else
                                        if (@rend="align(center) underline") then
                                            latex:block($config, ., ("tei-title20", css:map-rend-to-class(.)), .)
                                        else
                                            if (@rend="align(center)") then
                                                latex:block($config, ., ("tei-title21", css:map-rend-to-class(.)), .)
                                            else
                                                if ($parameters?mode='metadata-fr') then
                                                    latex:inline($config, ., ("tei-title22", css:map-rend-to-class(.)), .)
                                                else
                                                    if ($parameters?mode='metadata-en') then
                                                        latex:inline($config, ., ("tei-title23", css:map-rend-to-class(.)), .)
                                                    else
                                                        if ($parameters?mode='metadata-de') then
                                                            latex:inline($config, ., ("tei-title24", css:map-rend-to-class(.)), .)
                                                        else
                                                            if ($parameters?header='short') then
                                                                latex:heading($config, ., ("tei-title25", css:map-rend-to-class(.)), ., 5)
                                                            else
                                                                if (parent::titleStmt/parent::fileDesc) then
                                                                    (
                                                                        if (preceding-sibling::title) then
                                                                            latex:text($config, ., ("tei-title26", css:map-rend-to-class(.)), ' — ')
                                                                        else
                                                                            (),
                                                                        latex:inline($config, ., ("tei-title27", css:map-rend-to-class(.)), .)
                                                                    )

                                                                else
                                                                    if (not(@level)) then
                                                                        latex:inline($config, ., ("tei-title28", css:map-rend-to-class(.)), .)
                                                                    else
                                                                        if (@level='m' or not(@level)) then
                                                                            (
                                                                                latex:inline($config, ., ("tei-title29", css:map-rend-to-class(.)), .),
                                                                                if (ancestor::biblFull) then
                                                                                    latex:text($config, ., ("tei-title30", css:map-rend-to-class(.)), ', ')
                                                                                else
                                                                                    ()
                                                                            )
                                                                            ...
                                                                            if (@level='s' or @level='j') then
                                                                                (
                                                                                    latex:inline($config, ., ("tei-title31", css:map-rend-to-class(.)), .),
                                                                                    if (following-sibling::* and     (  ancestor::biblFull)) then
                                                                                        latex:text($config, ., ("tei-title32", css:map-rend-to-class(.)), ', ')
                                                                                    else
                                                                                        ()
                                                                                )
                                                                                ...
                                                                                if (@level='u' or @level='a') then
                                                                                    (
                                                                                        latex:inline($config, ., ("tei-title33", css:map-rend-to-class(.)), .),
                                                                                        if (following-sibling::* and     (    ancestor::biblFull)) then
                                                                                            latex:text($config, ., ("tei-title34", css:map-rend-to-class(.)), '. ')
                                                                                        else
                                                                                            ()
                                                                                    )
                                                                                    ...
                                                                                    if (contains(@rend,"underline")) then
                                                                                        let $params := 
                                                                                            map {
                                                                                                "content": .
                                                                                            }
                                                                                                                                                                                ...
                                                                                                                                                                                let $content := 
                                                                                            model:template-title3($config, ., $params)
                                                                                        return
                                                                                                                                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-title3", css:map-rend-to-class(.)), $content)
                                                                                    else
                                                                                        if (contains(@rend,"align(center)")) then
                                                                                            let $params := 
                                                                                                map {
                                                                                                    "content": .
                                                                                                }
                                                                                                                                                                                        ...
                                                                                                                                                                                        let $content := 
                                                                                                model:template-title4($config, ., $params)
                                                                                            return
                                                                                                                                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-title4", css:map-rend-to-class(.)), $content)
                                                                                        else
                                                                                            if (contains(@rend,"align(left)")) then
                                                                                                let $params := 
                                                                                                    map {
                                                                                                      "content": .
                                                                                                    }
                                                                                                                                                                                                ...
                                                                                                                                                                                                let $content := 
                                                                                                    model:template-title5($config, ., $params)
                                                                                                return
                                                                                                                                                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-title5", css:map-rend-to-class(.)), $content)
                                                                                            else
                                                                                                if (contains(@rend,"align(right)")) then
                                                                                                    let $params := 
                                                                                                      map {
                                                                                                      "content": .
                                                                                                      }
                                                                                                                                                                                                        ...
                                                                                                                                                                                                        let $content := 
                                                                                                      model:template-title6($config, ., $params)
                                                                                                    return
                                                                                                                                                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-title6", css:map-rend-to-class(.)), $content)
                                                                                                else
                                                                                                    if (not(@rend)) then
                    ...
                    case element(cit) return
                        if (child::quote and child::bibl ) then
                            ...
                            return
                                                        latex:cit(map:merge(($config, map:entry("template", true()))), ., ("tei-cit1", css:map-rend-to-class(.)), $content, ())
                        else
                            if (child::quote and child::bibl) then
                                (: Insert citation :)
                                latex:cit($config, ., ("tei-cit2", css:map-rend-to-class(.)), ., ())
                            else
                                $config?apply($config, ./node())
                    case element(titleStmt) return
                        if ($parameters?mode='metadata-fr') then
                            ...
                            if ($parameters?mode='metadata-en') then
                                latex:block($config, ., ("tei-titleStmt2", css:map-rend-to-class(.)), .)
                            else
                                if ($parameters?mode='metadata-de') then
                                    latex:block($config, ., ("tei-titleStmt3", css:map-rend-to-class(.)), .)
                                else
                                    if ($parameters?mode='title') then
                                        latex:heading($config, ., ("tei-titleStmt6", css:map-rend-to-class(.)), if (title[@xml:lang="en"]) then (title[@xml:lang="en"]) else (title[1]), 5)
                                    else
                                        $config?apply($config, ./node())
                        ...
                        if ($parameters?mode='list') then
                            (
                                if (descendant::listEvent) then
                                    latex:heading($config, ., ("tei-body4", css:map-rend-to-class(.)), "EVENTS", ())
                                else
                                    (),
                                if (descendant::listEvent) then
                                    latex:block($config, ., ("tei-body5", css:map-rend-to-class(.)), for $n in .//listEvent/event group by $ref := $n/label order by $ref return $n)
                                else
                                    (),
                                if (descendant::listPerson/@xml:id) then
                                    latex:heading($config, ., ("tei-body6", css:map-rend-to-class(.)), upper-case(listPerson/@xml:id), ())
                                else
                                    (),
                                if (descendant::listPlace/@xml:id) then
                                    latex:heading($config, ., ("tei-body7", css:map-rend-to-class(.)), upper-case(listPlace/@xml:id), ())
                                else
                                    (),
                                if (.//place[@type=('archeological_site')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body8", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('boulevard')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body9", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('building')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body10", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('cabaret')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body11", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('camp')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body12", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('canal')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body13", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('canton')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body14", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('castel')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body15", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('castle')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body16", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('cathedral')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body17", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('ccuntry')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body18", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('church')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body19", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('circus')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body20", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('city')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body21", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('college')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body22", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('continent')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body23", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('country')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body24", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('department')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body25", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('district')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body26", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('duchy')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body27", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('embassy')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body28", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('empire')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body29", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('forest')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body30", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('fort')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body31", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('gulf')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body32", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('hamlet')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body33", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('hill')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body34", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('hotel')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body35", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('house')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body36", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('island')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body37", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('lake')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body38", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('location')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body39", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('mansion')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body40", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('monastery')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body41", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('monument')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body42", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('mountain')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body43", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('mountains')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body44", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('municipality')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body45", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('museum')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body46", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('ocean')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body47", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('opera')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body48", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('park')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body49", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('pass')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body50", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('peninsula')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body51", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('place')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body52", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('plain')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body53", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('point')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body54", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('port')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body55", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('quarter')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body56", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('region')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body57", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('river')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body58", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('sea')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body59", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('settlement')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body60", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('square')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body61", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('state')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body62", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('station')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body63", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('steam')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body64", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('strait')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body65", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('stream')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body66", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('street')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body67", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('theater')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body68", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('thoroughfare')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body69", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('town')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body70", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('townhall')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body71", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('university')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body72", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('upland')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body73", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('valley')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body74", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[@type=('venue')]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body75", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (.//place[not(@type)]) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-body76", css:map-rend-to-class(.)), $content)
                                else
                                    ()
                            )
                            ...
                            if ($parameters?mode='index') then
                                (
                                    latex:block($config, ., ("tei-body77", css:map-rend-to-class(.)), for $n in .//listEvent/event group by $ref := $n/label order by $ref return $n),
                                    latex:block($config, ., ("tei-body78", css:map-rend-to-class(.)), for $n in .//place let $type := $n/@type group by $ref := $n/placeName[1] order by   ($type eq 'continent') descending,   ($type eq 'country') descending,   ($type eq 'region') descending,   ($type eq 'city') descending,   ($type eq 'mountain') descending,   ($type eq 'island') descending,   ($type eq 'opera') descending,   ($type eq 'college') descending,   ($type eq 'state') descending,   ($type eq 'church') descending,   ($type eq 'street') descending,   $ref return $n),
                                    latex:block($config, ., ("tei-body79", css:map-rend-to-class(.)), for $n in .//person group by $ref := $n/persName[1] order by $ref return $n),
                                    latex:block($config, ., ("tei-body80", css:map-rend-to-class(.)), for $n in .//org group by $ref := $n/orgName[1] order by $ref return $n),
                                    latex:block($config, ., ("tei-body81", css:map-rend-to-class(.)), for $n in .//biblStruct group by $ref := $n order by $ref return $n),
                                    latex:block($config, ., ("tei-body82", css:map-rend-to-class(.)), for $n in .//item group by $ref := $n/(name[1]|label[1]) order by $ref return $n)
                                )

                            else
                                (
                                    latex:index($config, ., ("tei-body83", css:map-rend-to-class(.)), ., 'toc'),
                                    latex:block($config, ., ("tei-body84", css:map-rend-to-class(.)), .)
                                )
                    ...
                    case element(fw) return
                        if (@type='letterhead' and @place="top(right)") then
                            ...
                            return
                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-fw1", css:map-rend-to-class(.)), $content)
                        else
                            if (@type='letterhead' and @place="top(center)") then
                                ...
                                return
                                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-fw2", css:map-rend-to-class(.)), $content)
                            else
                                if (@type='letterhead') then
                                    latex:block($config, ., ("tei-fw3", css:map-rend-to-class(.)), .)
                                else
                                    if (ancestor::opener or ancestor::div) then
                                        latex:inline($config, ., ("tei-fw4", css:map-rend-to-class(.)), .)
                                    else
                                        latex:block($config, ., ("tei-fw5", css:map-rend-to-class(.)), .)
                                ...
                                let $params := 
                                    map {
                                        "content": .
                                                                ...
                                                                let $content := 
                                    model:template-addrLine($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-addrLine1", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@rend="align(left)" or @rend="margin") then
                                let $params := 
                                    map {
                                        "content": .
                                    }
                                                                ...
                                                                let $content := 
                                    model:template-addrLine2($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-addrLine2", css:map-rend-to-class(.)), $content)
                            else
                                (),
                            if (@rend="indent" or @rend="align(center)") then
                                let $params := 
                                    map {
                                        "content": .
                                    }
                                                                ...
                                                                let $content := 
                                    model:template-addrLine3($config, ., $params)
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-addrLine3", css:map-rend-to-class(.)), $content)
                            else
                                ()
                        )
                        ...
                        if ($parameters?mode='original' or $parameters?mode='entities') then
                            ...
                            if (desc and $parameters?mode='corrections') then
                                ...
                                if (@extent and $parameters?mode='corrections') then
                                    latex:alternate($config, ., ("tei-gap6", css:map-rend-to-class(.)), ., @extent, "This is a gap in the text")
                                else
                                    if (not(@extent) and $parameters?mode='corrections') then
                                        latex:alternate($config, ., ("tei-gap7", css:map-rend-to-class(.)), ., "[[...]]", "This is a gap in the text")
                                    else
                                        (: latex rendition :)
                                        let $params := 
                                            map {
                                        ...
                                        return
                                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-gap2", css:map-rend-to-class(.)), $content)
                    case element(quote) return
                        if (ancestor::p) then
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., css:get-rendition(., ("tei-quote1", css:map-rend-to-class(.))), $content)
                        else
                            (: If it is inside a paragraph then it is inline, otherwise it is block level :)
                            let $params := 
                                map {
                            ...
                            return
                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., css:get-rendition(., ("tei-quote2", css:map-rend-to-class(.))), $content)
                        ...
                        if (parent::education or parent::affiliation) then
                            ...
                            if ((parent::desc or parent::persName) and ancestor::org) then
                                ...
                                if (ancestor::person) then
                                    latex:block($config, ., ("tei-persName3", css:map-rend-to-class(.)), .)
                                else
                                    if (($parameters?mode="index" or $parameters?mode="facets") and ancestor::org) then
                                        ...
                                        return
                                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-persName4", css:map-rend-to-class(.)), $content)
                                    else
                                        if ($parameters?mode='facets') then
                                            (: No function found for behavior: webcomponent :)
                                            $config?apply($config, ./node())
                        ...
                        if ($parameters?mode='index') then
                            (
                                ...
                                if (child::persName) then
                                    latex:block($config, ., ("tei-person3", css:map-rend-to-class(.)), persName except persName[1])
                                else
                                    (),
                                if (child::note) then
                                    latex:block($config, ., ("tei-person4", css:map-rend-to-class(.)), note/p)
                                else
                                    (),
                                if (child::birth) then
                                    let $params := 
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-person5", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (child::death) then
                                    let $params := 
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-person6", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (child::nationality) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-person7", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (child::sex) then
                                    let $params := 
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-person8", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (child::occupation) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-person9", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (child::education) then
                                    latex:block($config, ., ("tei-person10", css:map-rend-to-class(.)), education)
                                else
                                    (),
                                if (child::affiliation) then
                                    latex:block($config, ., ("tei-person11", css:map-rend-to-class(.)), affiliation)
                                else
                                    (),
                                if (child::event) then
                                    latex:block($config, ., ("tei-person12", css:map-rend-to-class(.)), event)
                                else
                                    (),
                                if (child::bibl) then
                                    latex:block($config, ., ("tei-person13", css:map-rend-to-class(.)), bibl)
                                else
                                    ()
                            )
                            ...
                            if ($parameters?mode='facets') then
                                (
                                    ...
                                    if (child::idno) then
                                        ...
                                        (),
                                    if (child::note) then
                                        latex:block($config, ., ("tei-person16", css:map-rend-to-class(.)), note/p)
                                    else
                                        (),
                                    if (child::birth) then
                                        let $params := 
                                        ...
                                        return
                                                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-person17", css:map-rend-to-class(.)), $content)
                                    else
                                        (),
                                    if (child::death) then
                                        let $params := 
                                        ...
                                        return
                                                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-person18", css:map-rend-to-class(.)), $content)
                                    else
                                        (),
                                    if (child::nationality) then
                                        let $params := 
                                        ...
                                        return
                                                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-person19", css:map-rend-to-class(.)), $content)
                                    else
                                        (),
                                    if (child::sex) then
                                        let $params := 
                                        ...
                                        return
                                                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-person20", css:map-rend-to-class(.)), $content)
                                    else
                                        (),
                                    if (child::occupation) then
                                        let $params := 
                                        ...
                                        return
                                                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-person21", css:map-rend-to-class(.)), $content)
                                    else
                                        (),
                                    if (child::education) then
                                        latex:block($config, ., ("tei-person22", css:map-rend-to-class(.)), education)
                                    else
                                        (),
                                    if (child::affiliation) then
                                        latex:block($config, ., ("tei-person23", css:map-rend-to-class(.)), affiliation)
                                    else
                                        (),
                                    if (child::event) then
                                        latex:block($config, ., ("tei-person24", css:map-rend-to-class(.)), event)
                                    else
                                        ()
                                )
                                ...
                                if ($parameters?mode="relations") then
                                    latex:omit($config, ., ("tei-person25", css:map-rend-to-class(.)), .)
                                else
                                    if ($parameters?mode='list') then
                                        (: No function found for behavior: webcomponent :)
                                        $config?apply($config, ./node())
                    ...
                    case element(placeName) return
                        if (parent::education or parent::affiliation) then
                            latex:inline($config, ., ("tei-placeName1", css:map-rend-to-class(.)), .)
                        else
                            if ($parameters?mode="index" and (parent::birth or parent::death)) then
                                ...
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-placeName2", css:map-rend-to-class(.)), $content)
                            else
                                if (ancestor::person or ancestor::place) then
                                    latex:inline($config, ., ("tei-placeName3", css:map-rend-to-class(.)), .)
                                else
                                    if ($parameters?mode="entities" and id(substring-after(@ref, '#'), root($parameters?root))/location/geo) then
                                        latex:omit($config, ., ("tei-placeName4", css:map-rend-to-class(.)), .)
                                    else
                                                        ...
                                                        if ($parameters?mode="entities" and @type="tgb" and id(substring-after(@ref, '#'), doc("/db/apps/discholed/data/rochlitz/index/indices.xml"))/location/geo) then
                                                            ...
                                                            if ($parameters?mode="entities" and @type="pec" and id(substring-after(@ref, '#'), doc("/db/apps/discholed/data/pec/index/Index_place.xml"))/location/geo) then
                                                                ...
                                                                if (@type="pec" and $parameters?mode='entities') then
                                                                    latex:alternate($config, ., ("tei-placeName11", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), doc("/db/apps/discholed/data/pec/index/Index_place.xml")))
                                                                else
                                                                    if (@type="bi" and $parameters?mode='entities') then
                                                                        latex:alternate($config, ., ("tei-placeName12", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), doc("/db/apps/discholed/data/bi/indexes/Ortsindex.xml")))
                                                                    else
                                                                        if (parent::org) then
                                                                            let $params := 
                                                                                map {
                                                                            ...
                                                                            return
                                                                                                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-placeName13", css:map-rend-to-class(.)), $content)
                                                                        else
                                                                            latex:inline($config, ., ("tei-placeName14", css:map-rend-to-class(.)), .)
                    ...
                    case element(affiliation) return
                        if (ancestor::org) then
                            ...
                            latex:inline($config, ., ("tei-affiliation1", css:map-rend-to-class(.)), .)
                        else
                            if (@when-iso) then
                                ...
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-affiliation2", css:map-rend-to-class(.)), $content)
                            else
                                if (@from-iso) then
                                    ...
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-affiliation3", css:map-rend-to-class(.)), $content)
                                else
                                    (: default rendition :)
                                    let $params := 
                                        map {
                                    ...
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-affiliation4", css:map-rend-to-class(.)), $content)
                        ...
                        if ($parameters?mode='index' and parent::listEvent) then
                            ...
                            if ($parameters?mode='list' and parent::listEvent) then
                                ...
                                if (@when-iso) then
                                    ...
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-event3", css:map-rend-to-class(.)), $content)
                                else
                                    let $params := 
                                        map {
                                            "content": ./p
                                    ...
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-event4", css:map-rend-to-class(.)), $content)
                    case element(place) return
                        if ($parameters?mode='index' and location/geo) then
                            (
                                (: No function found for behavior: webcomponent :)
                                $config?apply($config, ./node()),
                                latex:block($config, ., ("tei-place7", css:map-rend-to-class(.)), string-join(* except (location|idno), ', ')),
                                latex:block($config, ., ("tei-place8", css:map-rend-to-class(.)), string-join(location/address, ', ')),
                                if (location/geo) then
                                    latex:block($config, ., ("tei-place9", css:map-rend-to-class(.)), location/geo)
                                else
                                    (),
                                if (child::bibl) then
                                    latex:block($config, ., ("tei-place10", css:map-rend-to-class(.)), bibl)
                                else
                                    ()
                            )
                        ...
                        if ($parameters?mode='facets') then
                            (
                                ...
                                if (child::idno) then
                                    ...
                                    (),
                                if (desc) then
                                    latex:block($config, ., ("tei-org3", css:map-rend-to-class(.)), desc[1])
                                else
                                    (),
                                if (note) then
                                    latex:block($config, ., ("tei-org4", css:map-rend-to-class(.)), note)
                                else
                                    (),
                                if (child::placeName/@from-iso) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-org5", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (child::placeName/not(@from-iso)) then
                                    ...
                                    return
                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-org6", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (placeName/location/geo) then
                                    latex:block($config, ., ("tei-org7", css:map-rend-to-class(.)), placeName/location/geo)
                                else
                                    (),
                                if (child::persName) then
                                    let $params := 
                                    ...
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-org8", css:map-rend-to-class(.)), $content)
                                else
                                    ()
                            )
                            ...
                            if ($parameters?mode='index') then
                                (
                                    ...
                                    if (child::idno) then
                                        ...
                                        (),
                                    if (desc) then
                                        latex:block($config, ., ("tei-org11", css:map-rend-to-class(.)), desc[1])
                                    else
                                        (),
                                    if (note) then
                                        latex:block($config, ., ("tei-org12", css:map-rend-to-class(.)), note)
                                    else
                                        (),
                                    if (child::placeName/@from-iso) then
                                        let $params := 
                                        ...
                                        return
                                                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-org13", css:map-rend-to-class(.)), $content)
                                    else
                                        (),
                                    if (child::placeName/not(@from-iso)) then
                                        let $params := 
                                        ...
                                        return
                                                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-org14", css:map-rend-to-class(.)), $content)
                                    else
                                        (),
                                    if (placeName/location/geo) then
                                        latex:block($config, ., ("tei-org15", css:map-rend-to-class(.)), placeName/location/geo)
                                    else
                                        (),
                                    if (child::persName) then
                                        let $params := 
                                        ...
                                        return
                                                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-org16", css:map-rend-to-class(.)), $content)
                                    else
                                        ()
                                )
                                ...
                                if ($parameters?mode='facets') then
                                    ...
                                    if ($parameters?mode='entities' and ((@type="bi" and id(substring-after(@ref, '#'), doc("/db/apps/discholed/data/bi/indexes/Organisationsindex_Gruppenindex.xml"))) or (@type="pec" and id(substring-after(@ref, '#'), doc("/db/apps/discholed/data/pec/index/Index_organizations.xml"))) or (@type="ehri" and id(substring-after(@ref, '#'), doc("/db/apps/discholed/data/ehri/index/EHRI_index_organizations.xml"))))) then
                                        ...
                                        if (@from-iso) then
                                            ...
                                            return
                                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-orgName5", css:map-rend-to-class(.)), $content)
                                        else
                                            $config?apply($config, ./node())
                    case element(sex) return
                        if (@value="2") then
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-sex1", css:map-rend-to-class(.)), $content)
                        else
                            if (@value="1") then
                                let $params := 
                                    map {
                                ...
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-sex2", css:map-rend-to-class(.)), $content)
                            else
                                $config?apply($config, ./node())
                    ...
                    case element(specDesc) return
                        ...
                        return
                                                latex:row(map:merge(($config, map:entry("template", true()))), ., ("tei-specDesc", css:map-rend-to-class(.)), $content)
                    case element(specList) return
                        ...
                        return
                                                latex:list(map:merge(($config, map:entry("template", true()))), ., ("tei-specList", css:map-rend-to-class(.)), $content, ())
                    case element(egXML) return
                        latex:block($config, ., ("tei-egXML3", "programlisting", css:map-rend-to-class(.)), .)
                    case element(att) return
                        (: default rendition :)
                        latex:inline($config, ., ("tei-att", css:map-rend-to-class(.)), .)
                    case element(schemaSpec) return
                        ...
                        return
                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-schemaSpec", css:map-rend-to-class(.)), $content)
                        ...
                        if (parent::schemaSpec) then
                            (
                                if (@except="") then
                                    ...
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-moduleRef1", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (@except and not(@except="")) then
                                    ...
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-moduleRef2", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (@include) then
                                    let $params := 
                                    ...
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-moduleRef3", css:map-rend-to-class(.)), $content)
                                else
                                    ()
                            )
                    ...
                    case element(elementSpec) return
                        if (@mode='change') then
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-elementSpec", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(idno) return
                        if (@type="GND") then
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-idno1", css:map-rend-to-class(.)), $content)
                        else
                            if (@type="VIAF") then
                                let $params := 
                                    map {
                                ...
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-idno2", css:map-rend-to-class(.)), $content)
                            else
                                if (@type="PND") then
                                    let $params := 
                                        map {
                                    ...
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-idno3", css:map-rend-to-class(.)), $content)
                                else
                                    $config?apply($config, ./node())
                    case element(stamp) return
                        if (ancestor::additions) then
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-stamp1", css:map-rend-to-class(.)), $content)
                        else
                            let $params := 
                                map {
                                    "rend": if (@rend="align(center)") then 'text-align:center;' else  (if ((@rend="align(right)") or (@rend="margin(right)")) then 'text-align:right;' else     (if (@rend="align(left)") then 'text-align:left;' else         (if (@rend="indent") then 'text-indent: 15%;' else ''))),
                            ...
                            return
                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-stamp2", css:map-rend-to-class(.)), $content)
                    case element(change) return
                        if ($parameters?mode='citation') then
                            (: rendition for citation mode :)
                            latex:inline($config, ., ("tei-change", css:map-rend-to-class(.)), format-date(@when-iso, '[MNn] [D10], [Y]', 'en', 'AD', 'GB'))
                        else
                            $config?apply($config, ./node())
                    case element(relation) return
                        if (@mutual and $parameters?mode="index") then
                            ...
                            return
                                                        latex:omit(map:merge(($config, map:entry("template", true()))), ., ("tei-relation", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                        ...
                        if ($parameters?mode='index') then
                            (
                                if (child::author) then
                                    latex:inline($config, ., ("tei-monogr1", css:map-rend-to-class(.)), string-join(author, ", "))
                                else
                                    (),
                                if (child::editor) then
                                    latex:inline($config, ., ("tei-monogr2", css:map-rend-to-class(.)), string-join(editor, ", "))
                                else
                                    (),
                                if (child::title) then
                                    latex:inline($config, ., ("tei-monogr3", css:map-rend-to-class(.)), string-join(title, ", "))
                                else
                                    (),
                                if (child::imprint) then
                                    latex:inline($config, ., ("tei-monogr4", css:map-rend-to-class(.)), imprint)
                                else
                                    ()
                            )
                            ...
                            if ($parameters?mode='facets') then
                                (
                                    if (child::author) then
                                        latex:inline($config, ., ("tei-monogr5", css:map-rend-to-class(.)), string-join(author, ", "))
                                    else
                                        (),
                                    if (child::editor) then
                                        latex:inline($config, ., ("tei-monogr6", css:map-rend-to-class(.)), string-join(editor, ", "))
                                    else
                                        (),
                                    if (child::title) then
                                        latex:inline($config, ., ("tei-monogr7", css:map-rend-to-class(.)), string-join(title, ", "))
                                    else
                                        (),
                                    if (child::imprint) then
                                        latex:inline($config, ., ("tei-monogr8", css:map-rend-to-class(.)), imprint)
                                    else
                                        ()
                                )
                                ...
                                if ($parameters?mode='list') then
                                    (
                                        if (child::author) then
                                            latex:inline($config, ., ("tei-monogr9", css:map-rend-to-class(.)), string-join(author, ", "))
                                        else
                                            (),
                                        if (child::editor and not(child::author)) then
                                            latex:inline($config, ., ("tei-monogr10", css:map-rend-to-class(.)), string-join(editor, ", "))
                                        else
                                            (),
                                        if (child::title) then
                                            latex:inline($config, ., ("tei-monogr11", css:map-rend-to-class(.)), title[1])
                                        else
                                            ()
                                    )
                    ...
                    case element(biblStruct) return
                        if ($parameters?mode='index') then
                            ...
                            if ($parameters?mode='list') then
                                ...
                                (: default rendition :)
                                latex:block($config, ., ("tei-biblStruct3", css:map-rend-to-class(.)), .)
                        ...
                        if ($parameters?mode='index' or $parameters?mode='list' or $parameters?mode='facets') then
                            (
                                if (child::pubPlace/text()) then
                                    latex:inline($config, ., ("tei-imprint1", css:map-rend-to-class(.)), pubPlace)
                                else
                                    (),
                                if (child::publisher/text()) then
                                    latex:inline($config, ., ("tei-imprint2", css:map-rend-to-class(.)), publisher)
                                else
                                    (),
                                if (child::date[@from-iso] or child::date[@notBefore-iso]) then
                                    latex:inline($config, ., ("tei-imprint3", css:map-rend-to-class(.)), date)
                                else
                                    (),
                                if (child::date[@when-iso]) then
                                    latex:inline($config, ., ("tei-imprint4", css:map-rend-to-class(.)), if (date/text()) then (date/text()) else (date/when-iso))
                                else
                                    (),
                                if (child::date[@when]) then
                                    latex:inline($config, ., ("tei-imprint5", css:map-rend-to-class(.)), if (date/text()) then (date/text()) else (date/when))
                                else
                                    (),
                                if (child::biblScope[@unit="page"]) then
                                    ...
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-imprint6", css:map-rend-to-class(.)), $content)
                                else
                                    (),
                                if (child::biblScope[@unit="volume"]) then
                                    ...
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-imprint7", css:map-rend-to-class(.)), $content)
                                else
                                    ()
                            )
                    ...
                    case element(analytic) return
                        if ($parameters?mode='index') then
                            (
                                if (child::author) then
                                    latex:inline($config, ., ("tei-analytic1", css:map-rend-to-class(.)), string-join(author, ", "))
                                else
                                    (),
                                if (child::editor) then
                                    latex:inline($config, ., ("tei-analytic2", css:map-rend-to-class(.)), string-join(editor, ", "))
                                else
                                    (),
                                if (child::title) then
                                    latex:inline($config, ., ("tei-analytic3", css:map-rend-to-class(.)), string-join(title, ", "))
                                else
                                    ()
                            )
                            ...
                            if ($parameters?mode='list') then
                                (
                                    if (child::author) then
                                        latex:inline($config, ., ("tei-analytic4", css:map-rend-to-class(.)), string-join(author, ", "))
                                    else
                                        (),
                                    if (child::editor) then
                                        latex:inline($config, ., ("tei-analytic5", css:map-rend-to-class(.)), string-join(editor, ", "))
                                    else
                                        (),
                                    if (child::title) then
                                        latex:inline($config, ., ("tei-analytic6", css:map-rend-to-class(.)), string-join(title, ", "))
                                    else
                                        ()
                                )
                            ...
                            let $params := 
                                map {
                                    "content": if (@style="x") then 'x' else   (if (@style="horizontal_rule") then '____________________' else    (if (@style="crosses") then '  +   +   +' else     (if (@style="dots") then '..........' else      (if (@style="line") then '----------' else       (if (@style="double_line") then '==========' else        (if (@style="vertical_rule") then '|' else         (if (@style="curved_horizontal_line") then '~~~' else                              '')))))))
                                                        ...
                                                        let $content := 
                                model:template-metamark($config, ., $params)
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-metamark1", css:map-rend-to-class(.)), $content)
                        )

                    case element(emph) return
                        latex:inline($config, ., ("tei-emph", css:map-rend-to-class(.)), .)
                    case element(handShift) return
                        latex:alternate($config, ., ("tei-handShift", css:map-rend-to-class(.)), ., "[Shift in the document hand]", id(substring-after(@new, '#'), root($parameters?root))/p[@xml:lang="en"])
                    case element(delSpan) return
                        latex:alternate($config, ., ("tei-delSpan", css:map-rend-to-class(.)), ., "[From here, a large part of the text has been strikethrough]", id(substring-after(@hand, '#'), root($parameters?root))/p[@xml:lang="en"])
                    case element(education) return
                        ...
                        return
                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-education", css:map-rend-to-class(.)), $content)
                    case element(location) return
                        (
                            if (/geo) then
                                latex:alternate($config, ., ("tei-location", css:map-rend-to-class(.)), ., /geo, let $address := ../location/address let $placenames := string-join(../placeName/text(), ', ') let $geo := /geo  let $country := string-join(../country, ', ') return <div class='card_index'> {   if ($country) then     <p>{$country}</p>   else (),   if ($placenames) then     <p>{$placenames}</p>   else (),   if ($address) then     <p>Address:       {for $child in $address/*       return concat(string($child), ', ')}     </p>   else (),   if ($geo) then     <p>Location: <span style='color: blue;'>{$geo}</span></p>   else () } </div>)
                            else
                                ()
                        )
