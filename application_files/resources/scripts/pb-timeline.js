...
/**
 * A timline component to display time series data in a bar chart like view.
 *
 ...
 * Sample Usage:
 * ```xml
 * <pb-timline url="api/timline" scopes="['D', 'M', 'Y', '5Y', '10Y']"
 *      resettable=""
 *      subscribe="docs" emit="timline">
 ...
 * - [templates/people.html](https://github.com/stazh/briefedition-escher/blob/master/templates/people.html#L91) - usage of pb-timline
 * - [modules/custom-api.json](https://github.com/stazh/briefedition-escher/blob/master/modules/custom-api.json#L1080) - `/api/timline` endpoint delivering required JSON object
 * 
 * @slot label - Inserted before the label showing the currently displayed time range
 * 
 * @fires pb-timline-date-changed - Triggered when user clicks on a single entry
 * @fires pb-timline-daterange-changed - Triggered when user selects a range of entries
 * @fires pb-timline-reset-selection - Requests that the timline is reset to initial state
 * @fires pb-timline-loaded - timline was loaded
 * 
 * @cssprop --pb-timline-height
 * @cssprop --pb-timline-padding
 * @cssprop --pb-timline-color-highlight
 * @cssprop --pb-timline-color-light
 * @cssprop --pb-timline-color-dark
 * @cssprop --pb-timline-color-selected
 * @cssprop --pb-timline-color-bin
 * @cssprop --pb-timline-title-font-size
 * @cssprop --pb-timline-tooltip-font-size
 * 
 * @csspart label
 * @csspart tooltip
 * @csspart title
 */
export class Pbtimline extends pbMixin(LitElement) {
    ...
    return css`
      ...
      .wrapper {
        margin: 0 auto;
        padding: var(--pb-timline-padding);
        width: auto;
        height: var(--pb-timline-height, 80px);
        display: flex;
        position: relative;
      }
      .wrapper.empty {
      ...
      .bin-container {
        cursor: crosshair;
        margin-top: 20px;
        min-width: var(--pb-timline-min-width, 14px);
        max-width: var(--pb-timline-max-width, 20px);
        flex-grow: 1;
        flex-basis: 0;
        display: flex;
      ...
      .bin-container:hover .bin {
        background-color: var(--pb-timline-color-highlight, #3f52b5);
      }
      ...
      .bin-container.white {
        background-color: var(--pb-timline-color-light, white);
      }
      .bin-container.grey {
        background-color: var(--pb-timline-color-dark, #f1f1f1);
      }
      .bin-container.selected {
        background-color: var(--pb-timline-color-selected, #e6eaff) !important;
      }
      .bin {
        width: 80%;
        background-color: var(--pb-timline-color-bin, #ccc);
        border-radius: 2px;
        user-select: none;
      }
      p.bin-title {
        ...
        z-index: 10;
        margin: 0;
        font-size: var(--pb-timline-title-font-size, 12px);
        user-select: none;
        white-space: nowrap;
      }
      p.bin-title.months {
      ...
      .bins-title {
        ...
        top: -20px;
        font-size: var(--pb-timline-title-font-size, 12px);
        background-color: var(--pb-timline-background-color-title, #535353);
        color: var(--pb-timline-color-title, #ffffff);
        padding: 2px 4px;
        border-radius: 5px;
        height: var(--pb-timline-title-font-size, 12px);
        line-height: var(--pb-timline-title-font-size, 12px);
        user-select: none;
      }
      .info {
        display: none;
      ...
      #tooltip {
        ...
        position: absolute;
        min-width: var(--pb-timline-tooltip-min-width, 200px);
        font-size: var(--pb-timline-tooltip-font-size, 11px);
        line-height: 1.25;
        background: var(--pb-timline-background-color-title, #535353);
        color: var(--pb-timline-color-title, #ffffff);
        text-align: left;
        border-radius: 6px;
        padding: 5px 10px;
        top: var(--pb-timline-height, 80px);
        left: 0;
        z-index: 1000;
      }
      #tooltip ul {
    ...
    static get properties() {
        return {
          ...super.properties,
            /**
             * start date for timline to display
             */
            startDate:{
                type: String,
                reflect: true,
            ...
            /**
             * endDate for timline to display
             */
            endDate: {
                type: String,
                reflect: true,
            ...
            /**
             * The scope for the timline. Must be one of the pre-defined scopes.
            ...
            /**
             * Endpoint to load timline data from. Expects response to be an
  ...
  firstUpdated() {
    ...
    // pb-timline-daterange-changed event:
    ...
    document.addEventListener("pb-timline-daterange-changed", (event) => {
    ...
    // pb-timline-reset-selection:
    // resets selection (remove marking of all selected bins)
    // is triggered by the componeent itself but can be also triggered
    // from outside by another component
    document.addEventListener("pb-timline-reset-selection", () => {
      this.resetSelection();
      this._hideTooltip();
    });
  }
  ...
  _mouseUp() {
    if (this.mousedown) {
      ...
      if (start) {
        ...
        this._dispatchtimlineDaterangeChangedEvent(startDateStr, endDateStr, this.getSelectedCategories(), itemCount);
      }
    }
  }
  ...
  _dispatchtimlineDaterangeChangedEvent(startDateStr, endDateStr, categories, itemCount) {
    if (startDateStr === '????-??-??') {
      this.emitTo('pb-timline-date-changed', { startDateStr: null, endDateStr: null, categories: ['?'], count: itemCount });
    } else if(startDateStr === endDateStr) {
      if (this.dataObj.scope !== 'D') {
        this.emitTo('pb-timline-daterange-changed', {
          startDateStr,
        ...
        this.emitTo('pb-timline-date-changed', { 
          startDateStr, 
          endDateStr: null, 
          scope: this.dataObj.scope, 
          categories, 
  ...
  _dispatchPbtimlineResetSelectionEvent() {
    this.emitTo('pb-timline-reset-selection');
  }
      ...
      <div class="label" part="label">
        ...
        ${
          this.resettable ? html`
            <paper-icon-button id="clear" icon="icons:clear" title="${translate('timline.clear')}"
            @click="${this._dispatchPbtimlineResetSelectionEvent}"></paper-icon-button>
          ` : null
        }
      </div>
    ...
    return html`
      <div id="tooltip" class="hidden" part="tooltip">
        ...
        <div
          id="tooltip-close"
          class="hidden"
          @click="${this._dispatchPbtimlineResetSelectionEvent}"
  ...
  renderInfo(binObj) {
    if (binObj.info && binObj.info.length > 0) {
      return html`
        <ul class="info">
...
customElements.define('pb-timline', Pbtimline);
